  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="relative h-screen min-h-[600px] overflow-hidden">
        ...
        {/* ── Controls: bottom-right, well clear of text ── */}
        <div className="absolute bottom-8 right-6 z-30 flex items-center gap-3">
          <button onClick={() => { prevSlide(); resetTimer(); }}
            className="w-11 h-11 bg-white/15 backdrop-blur border border-white/25 text-white rounded-full flex items-center justify-center hover:bg-brand transition-all">
          ...
          <div className="flex gap-1.5">
            {SLIDES.map((_, i) => (
              <button key={i} onClick={() => { changeSlide(i, i > slide ? "next" : "prev"); resetTimer(); }}
                className={`rounded-full transition-all duration-400 ${i === slide ? "w-7 h-2.5 bg-brand-400" : "w-2.5 h-2.5 bg-white/40 hover:bg-white/70"}`} />
            ))}
          </div>

          <button onClick={() => { nextSlide(); resetTimer(); }}
            className="w-11 h-11 bg-white/15 backdrop-blur border border-white/25 text-white rounded-full flex items-center justify-center hover:bg-brand transition-all">
      ...
      <section className="py-24 bg-gray-950 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-3xl p-8 md:p-12 text-center mb-6">
            ...
            <div className={`transition-opacity duration-500 ease-in-out ${testFading ? "opacity-0" : "opacity-100"}`}>
